...
if [ ! -d /tmp/share ]; then
	mkdir /tmp/share
  	echo "Directory share created."
fi
ln -sf /tmp/share $HOME/share


## download directory
if [ ! -d /tmp/download ]; then
	mkdir /tmp/download
	echo "Directory download created."
fi
ln -sf /tmp/download $HOME/download



function update() {
    reflector --save /etc/pacman.d/mirrorlist --counrty Indonesia,Singapore --protocol https --latest 5 &&
    pacman -Syu &&
    pacman -Scc
}